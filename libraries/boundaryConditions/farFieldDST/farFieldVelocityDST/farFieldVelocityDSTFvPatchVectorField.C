...
#include "farFieldVelocityDSTFvPatchVectorField.H"
...
farFieldVelocityDSTFvPatchVectorField::farFieldVelocityDSTFvPatchVectorField
(
    const fvPatch& p,
    ...
    TInf_(readScalar(dict.lookup("TInf")))
{
    ...
    {
        fvPatchVectorField::operator=(this->refValue());
    }
...
farFieldVelocityDSTFvPatchVectorField::farFieldVelocityDSTFvPatchVectorField
(
    const farFieldVelocityDSTFvPatchVectorField& ptf,
    const fvPatch& p,
    const DimensionedField<vector, volMesh>& iF,
...
void farFieldVelocityDSTFvPatchVectorField::updateCoeffs()
{
    if (this->updated())
    {
        return;
    ...
    scalarField pb = 0.5*
        (
          ...
          - rho0c0
          * (
                n & (UInf_ - patchInternalField())
            )
        );
    vectorField subsInValue = UInf_ - n*(pInf_ - pb)/rho0c0;
    vectorField subsOutValue = n*(pI-pb)/rho0c0;
    
    this->refValue() = 
        (
            supersonicInflow * UInf_
          + subsonicInflow * subsInValue
          + subsonicOutflow * subsOutValue
        );
    
    this->valueFraction() = 1 - supersonicOutflow - subsonicOutflow;

    mixedFvPatchVectorField::updateCoeffs();
}


void farFieldVelocityDSTFvPatchVectorField::write(Ostream& os) const
{
...
makePatchTypeField(fvPatchVectorField, farFieldVelocityDSTFvPatchVectorField);
